      ...
      def typedNew(tpt: Tree) = {
        ...
        def narrowRhs(tp: Type) = {
          ...
          if (sym != null && sym != NoSymbol) {
            ...
            case TypeRef(pre, sym, List()) if sym.isModuleClass => // #3612: if we're instantiating a module class
              // (e.g., in the tree generated by mkCachedModuleAccessDef)
              // the result must be the singleton type of the corresponding module in order for the type of the new MC
              // to conform to the object's self type
              assert(sym.sourceModule != NoSymbol, sym)
              intersectionType(List(tp, singleType(pre, sym.sourceModule)))
            case _ => tp
          }
        }
