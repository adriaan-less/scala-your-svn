...
//        case Ident(name) => ??
      ...
      def typedNew(tpt: Tree) = {
        ...
        else if (!(  tpt1.tpe.typeSymbol.initialize.thisSym.tpe == tpt1.tpe // explicit self type means thisSym is different, but unless self type is given explicitly, thisSym.tpe is still equalt to tpt1.tpe
