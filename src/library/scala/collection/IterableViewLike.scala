...
{ self =>
  ...
  override def zip[A1 >: A, B, That](that: Iterable[B])(implicit bf: CanBuildFrom[(A1, B), That, This]): That = {
...
//    else super.zip[A1, B, That](that)(bf)    
  ...
  override def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[(A1, Int), That, This]): That =
    zip[A1, Int, That](Stream from 0)(bf)

  override def zipAll[B, A1 >: A, That](that: Iterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[(A1, B), That, This]): That =
