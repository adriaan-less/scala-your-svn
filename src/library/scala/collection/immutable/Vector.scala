...
object Vector extends SeqFactory[Vector] {
  implicit def builderFactory[A]: CanBuildFrom_Done[Coll, A, Vector[A]] = 
...
object NewVector extends SeqFactory[NewVector] {
  ...
  implicit def builderFactory[A]: CanBuildFrom_Done[Coll, A, NewVector[A]] =
    bf.asInstanceOf[CanBuildFrom_Done[Coll], A, NewVector[A]]
...
//    //println("get elem: "+index + "/"+idx + "(focus:" +focus+" xor:"+(idx^focus)+" depth:"+depth+")")
  ...
  override def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom_Done[NewVector[A], B, That]): That = {
    // just ignore bf
    updateAt(index, elem).asInstanceOf[That]
  }

  override def +:[B >: A, That](elem: B)(implicit bf: CanBuildFrom_Done[NewVector[A], B, That]): That = {
    // just ignore bf
