...
self =>
   ...
   */
  def zip[A1 >: A, B, That](that: Iterable[B])(implicit bf: CanBuildFrom[(A1, B), That, Repr]): That = {
    val b = bf(repr)
   ...
   *  
   */
  def zipAll[B, A1 >: A, That](that: Iterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[(A1, B), That, Repr]): That = {  
    val b = bf(repr)
    val these = this.iterator
    val those = that.iterator
    while (these.hasNext && those.hasNext)
   ...
   */
  def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[(A1, Int), That, Repr]): That = {
    val b = bf(repr)
    var i = 0
    for (x <- this) {
      b += ((x, i))
