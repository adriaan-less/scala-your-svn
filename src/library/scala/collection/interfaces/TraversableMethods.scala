...
trait TraversableMethods[+A, +This <: TraversableLike[A, This] with Traversable[A]]
{
  ...
  // maps/iteration
  def flatMap[B, That](f: A => Traversable[B])(implicit bf: CanBuildFrom_Done[This, B, That]): That
  def map[B, That](f: A => B)(implicit bf: CanBuildFrom_Done[This, B, That]): That
  def filterMap[B, That](pf: PartialFunction[Any, B])(implicit bf: CanBuildFrom_Done[This, B, That]): That
  
  // new collections
  def ++[B >: A, That](that: Iterator[B])(implicit bf: CanBuildFrom_Done[This, B, That]): That
  def ++[B >: A, That](that: Traversable[B])(implicit bf: CanBuildFrom_Done[This, B, That]): That
