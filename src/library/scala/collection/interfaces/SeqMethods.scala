...
trait SeqMethods[+A, +This <: SeqLike[A, This] with Seq[A]] extends IterableMethods[A, This]
{
  ...
  def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[B, That, This]): That
  def patch[B >: A, That](from: Int, patch: Seq[B], replaced: Int)(implicit bf: CanBuildFrom[B, That, This]): That 
  ...
  def union[B >: A, That](that: Seq[B])(implicit bf: CanBuildFrom[B, That, This]): That
