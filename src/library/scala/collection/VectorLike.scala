...
trait VectorLike[+A, +Repr] extends SeqLike[A, Repr] { self =>
  ...
  override def zip[A1 >: A, B, That](that: Iterable[B])(implicit bf: CanBuildFrom[(A1, B), That, Repr]): That = that match {
    case that: Vector[_] =>
      val b = bf(repr)
      var i = 0
  ...
  override def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[(A1, Int), That, Repr]): That = {
    val b = bf(repr)
    val len = length
    b.sizeHint(len)
    var i = 0
