...
self =>
   ...
   */
  def ++[B >: A, That](that: Traversable[B])(implicit bf: CanBuildFrom_Done[Repr, B, That]): That = {
    val b = bf(repr)
    b ++= thisCollection
    b ++= that
    b.result
   ...
   */
  def ++[B >: A, That](that: Iterator[B])(implicit bf: CanBuildFrom_Done[Repr, B, That]): That = {
    val b = bf(repr)
    b ++= thisCollection
    b ++= that
    b.result
   ...
   */
  def map[B, That](f: A => B)(implicit bf: CanBuildFrom_Done[Repr, B, That]): That = {
    val b = bf(repr)
    for (x <- this) b += f(x)
    b.result
  }
   ...
   */
  def flatMap[B, That](f: A => Traversable[B])(implicit bf: CanBuildFrom_Done[Repr, B, That]): That = {
    val b = bf(repr)
    for (x <- this) b ++= f(x)
    b.result
  }
  ...
  */
  @experimental
  def filterMap[B, That](pf: PartialFunction[Any, B])(implicit bf: CanBuildFrom_Done[Repr, B, That]): That = {
    val b = bf(repr)
