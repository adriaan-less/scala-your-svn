...
trait TraversableProxyLike[+A, +This <: TraversableLike[A, This] with Traversable[A]] extends TraversableLike[A, This] with Proxy {
  ...
  override def ++[B >: A, That](that: Traversable[B])(implicit bf: CanBuildFrom[B, That, This]): That = self.++(that)(bf)
  override def ++[B >: A, That](that: Iterator[B])(implicit bf: CanBuildFrom[B, That, This]): That = self.++(that)(bf)
  override def map[B, That](f: A => B)(implicit bf: CanBuildFrom[B, That, This]): That = self.map(f)(bf)
  override def flatMap[B, That](f: A => Traversable[B])(implicit bf: CanBuildFrom[B, That, This]): That = self.flatMap(f)(bf)
