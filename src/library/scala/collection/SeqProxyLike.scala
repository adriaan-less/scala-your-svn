...
trait SeqProxyLike[+A, +This <: SeqLike[A, This] with Seq[A]] extends SeqLike[A, This] with IterableProxyLike[A, This] { 
  ...
  override def union[B >: A, That](that: Seq[B])(implicit bf: CanBuildFrom_Done[This, B, That]): That = self.union(that)(bf)
  ...
  override def patch[B >: A, That](from: Int, patch: Seq[B], replaced: Int)(implicit bf: CanBuildFrom_Done[This, B, That]): That = self.patch(from, patch, replaced)(bf)
  override def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom_Done[This, B, That]): That = self.padTo(len, elem)(bf)
