...
trait SeqLike[+A, +Repr] extends IterableLike[A, Repr] { self =>
  ...
  */
   ...
   */
  def union[B >: A, That](that: Seq[B])(implicit bf: CanBuildFrom[B, That, Repr]): That = 
    this ++ that
   ...
   *  by `patch`.
   */
  def patch[B >: A, That](from: Int, patch: Seq[B], replaced: Int)(implicit bf: CanBuildFrom[B, That, Repr]): That = {
    val b = bf(repr)
   ...
   */
  def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[B, That, Repr]): That = {
    val b = bf(repr)
   ...
   */
  def +:[B >: A, That](elem: B)(implicit bf: CanBuildFrom[B, That, Repr]): That = {
    val b = bf(repr)
    b += elem
    b ++= thisCollection
    b.result
   ...
   */
  def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[B, That, Repr]): That = {
    val b = bf(repr)
