...
{   
  ...
  override def zip[A1 >: A, B, That](that: Iterable[B])(implicit bf: CanBuildFrom[(A1, B), That, This]): That = self.zip[A1, B, That](that)(bf)
  override def zipAll[B, A1 >: A, That](that: Iterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[(A1, B), That, This]): That = self.zipAll(that, thisElem, thatElem)(bf)
  override def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[(A1, Int), That, This]): That = self.zipWithIndex(bf)
